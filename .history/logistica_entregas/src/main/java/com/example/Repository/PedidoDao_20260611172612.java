...
public class PedidoDao 
{
    ...
    public static HashMap<Double, String> relatorioPedidoPendenteEstado() throws SQLException
    {
        HashMap<Double, String> pedidosPendentes = new HashMap<>();

        String sql = """
                SELECT 
                    SUM(p.id) AS totalPedidos,
                    c.estado AS ESTADOS
                FROM pedido p
                JOIN cliente c ON c.id = p.
                """;
    }
}
