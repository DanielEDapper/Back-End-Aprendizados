...
public class EntregaDao 
{
    ...
    public static Entrega buscarEntrega(int idEntrega) throws SQLException
    {
        ...
        LocalDateTime data_entrega = null;
        StatusEntrega statusEntrega = null;

        String sql = """
                SELECT id, pedido_id motorista_id, data-saida, data_entrega, status FROM entrega WHERE id = ?
                """;
            ...
            PreparedStatement stmt = conn.prepareStatement(sql))
        {
            ...
            if(rs.next())
            {
                ...
                motorista_id = rs.getInt("motorista_id");
                data_saida = rs.getObject("data_saida", LocalDateTime.class);
                data_entrega = rs.getObject("data_entrega", LocalDateTime.class);
                statusEntrega = StatusEntrega.valueOf(rs.getString("status"));
            }
        }
        return new Entrega(id, pedido_id, motorista_id, data_saida, data_entrega, statusEntrega);
    }
}
