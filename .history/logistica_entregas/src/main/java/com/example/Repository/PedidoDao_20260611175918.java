...
public class PedidoDao 
{
    ...
    public static Pedido buscarPedidoPorCpf(String cpf) throws SQLException
    {
        String sql = """
                SELECT 
        ...
        while (rs.next()) {
            ...
            // Uso obrigatório de java.time [4]
            LocalDateTime dataPedido = rs.getObject("data_pedido", LocalDateTime.class);
            double volume = rs.getDouble("volume_m3");
            double peso = rs.getDouble("peso_kg");
            // Conversão da String do banco para o seu Enum [2]
            StatusPedido status = StatusPedido.valueOf(rs.getString("status"));

            Pedido pedido = new Pedido(id, clienteId, dataPedido, volume, peso, status);
            pedidos.add(pedido);
        }
    }
    return pedidos;
