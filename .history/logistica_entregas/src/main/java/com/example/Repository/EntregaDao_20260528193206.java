    ...
    public static void atualizarEntrega(Entrega entrega) throws SQLException
    {
        String sql = """
                ...
                try(Connection conn = ConnectionFactory.conectar();
                    PreparedStatement stmt = conn.prepareStatement(sql))
                {
                    stmt.setInt(1, entrega.getIdEntrega());
                    stmt.setInt(2, entrega.getIdPedido());
                    stmt.setInt(3, entrega.getIdMotorista());
                    stmt.setObject(4, entrega.getDataSaida());
                    stmt.setObject(5, entrega.getDataEntrega());
                    stmt.setString(6, entrega.getStatusEntrega().name());
