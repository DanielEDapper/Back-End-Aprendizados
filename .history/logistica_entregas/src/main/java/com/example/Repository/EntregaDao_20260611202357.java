...
public class EntregaDao 
{
        ...
        try(Connection conn = ConnectionFactory.conectar();
            PreparedStatement stmt = conn.prepareStatement(sql))
            {
                ...
                while(rs.next())
                {
                    ...
                    entregas.put(cidade, totalEntrega);
                }
            }
        return entregas;
    }
    public static void cancelarEntrega(int idPedido) throws SQLException
    {
        String sql = """
    ...
    public void atualizarStatusNoBanco(int id, StatusEntrega novoStatus, LocalDateTime dataEntrega) throws SQLException {
    String sql = "UPDATE entrega SET status = ?, data_entrega = ? WHERE id = ?";
    
    try (Connection conn = ConnectionFactory.conectar();
         PreparedStatement stmt = conn.prepareStatement(sql)) {
        
        stmt.setString(1, novoStatus.name()); // Salva o nome do Enum (ex: EM_ROTA)
        stmt.setObject(2, dataEntrega);      // Salva a data (pode ser null se não for 'ENTREGUE')
        stmt.setInt(3, id);
